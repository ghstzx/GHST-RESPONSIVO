...
function update(skipTransition) {
  ...
  // ── centerBtn: só aparece nos slides especiais ──
  const specialSlide = slides[currentSlide];
  const isUniverso = specialSlide && specialSlide.classList.contains('slide-universo');
  const isMapa     = specialSlide && specialSlide.classList.contains('slide-mapa');
  const isConst    = specialSlide && specialSlide.classList.contains('slide-const');

  if (isUniverso || isMapa || isConst) {
    centerBtn.style.display = 'block';
    centerBtn.classList.remove('hidden');
    centerBtn._constReady = false;
    // universo e const: visível imediatamente; mapa: animação controla a aparição
    centerBtn.style.opacity = isMapa ? '0' : '1';
  } else {
...
(function () {
  ...
  function uvInit() {
    ...
    // Clique no botão central avança as fases
    centerBtn.onclick = uvAdvance;
...
(function () {
  ...
  new MutationObserver(() => {
    if (slide.classList.contains('active')) {
      // Reseta ao reentrar no slide (igual à constelação)
      ready = false;
      phase = 0;
      if (animId) { cancelAnimationFrame(animId); animId = null; }
      init();
      centerBtn.onclick = advance;
    }
  }).observe(slide, { attributes: true, attributeFilter: ['class'] });
  ...
  function startIntroLines() {
    ...
    // botão visível imediatamente, igual ao universo e constelação
  ...
  function advance() {
    if (phase===0) {
      phase=1;
      // botão permanece visível — usuário pode pular a animação a qualquer momento
      showPhase('mpMap');
...
(function () {
  ...
  // ─── Inicialização disparada pelo observer ───
  function initConst() {
    if (constInited) return;
    constInited = true;
    // Para qualquer animação anterior que possa ter ficado rodando
  ...
  // Liga o centerBtn quando o slide de constelação ficar ativo, e inicia animação
  const constSlideEl2 = slide;
  const constObserver = new MutationObserver(() => {
    if (constSlideEl2.classList.contains('active')) {
      // Reseta ao reentrar no slide (permite que o botão apareça toda vez)
      constInited = false;
      constPhase  = 0;
      initConst();
      centerBtn.onclick = constAdvance;
    }
  });
  constObserver.observe(constSlideEl2, { attributes: true, attributeFilter: ['class'] });
  ...
  function drawConstellation() {
    ...
    function frame() {
      ...
      if (tick >= DONE_TICK) {
        ...
        // botão já estava visível desde o início — não precisa fazer nada aqui
      }
