...
function update(skipTransition) {
  ...
  if (isUniverso || isMapa || isConst) {
    ...
    // Reseta estado visual a cada vez que entra no slide
    centerBtn.classList.remove('hidden');
    centerBtn._constReady = false;
    // universo: visível desde o início; mapa e const: esperam animação para aparecer
    centerBtn.style.opacity = isUniverso ? '1' : '0';
  } else {
  ...
  function drawConstellation() {
    ...
    function frame() {
      ...
      if (tick >= DONE_TICK) {
        ...
        // botão aparece para avançar para fase 2
        if (!centerBtn._constReady) {
          centerBtn._constReady = true;
          centerBtn.style.opacity = '1';
        }
      }
